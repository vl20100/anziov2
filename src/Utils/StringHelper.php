...
class StringHelper
{
     ...
     * @return bool
     */
    public function arePasswordEquals(?string $password, string $encrypted) : bool
    {
        if(is_null($password))
            return false;

        $new = $this->encryptPassword($password);
    ...
    {
        $str = strtolower($this->removeAccents($str));
        $tab = explode(" ", $str);
